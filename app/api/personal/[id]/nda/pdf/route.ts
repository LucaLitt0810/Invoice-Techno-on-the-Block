...
export async function GET(
  ...
  try {
    ...
    const drawBlankLine = (size: number) => {
      y -= size * 1.6;
    };

    // Title
    drawLine('VERSCHWIEGENHEITSVEREINBARUNG', 16, true);
    drawBlankLine(12);

    // Header
    drawTextWrapped('Zwischen dem "Verein Techno on the Block",', 10);
    drawTextWrapped('einem Verein nach Schweizer Recht,', 10);
    drawBlankLine(10);
    drawTextWrapped('vertreten durch seinen Vorstand:', 10);
    drawTextWrapped('Ben Littmann & Alina Littmann,', 10);
    drawTextWrapped('Pratteln, Vereinshausstrasse 10, 4133 Pratteln, Schweiz', 10);
    drawLine('nachstehend "Verein"', 10, true);
    drawBlankLine(10);

    // Employee name as contract party
    ...
    // Signatures
    page.drawText(`Pratteln, ${today}`, { x: margin, y, size: 10, font });
    y -= 30;

    const sigY = y;
    page.drawLine({ start: { x: margin, y: sigY }, end: { x: margin + 200, y: sigY }, thickness: 0.5 });
    page.drawLine({ start: { x: margin + 260, y: sigY }, end: { x: margin + 460, y: sigY }, thickness: 0.5 });
    y -= 14;

    page.drawText('Unterschrift Verein', { x: margin, y, size: 9, font });
    page.drawText('Unterschrift Vertragsnehmer', { x: margin + 260, y, size: 9, font });
    y -= 14;

    page.drawText('Ben Littmann & Alina Littmann', { x: margin, y, size: 9, font: fontBold });
    page.drawText(fullName, { x: margin + 260, y, size: 9, font: fontBold });
